...
### Design Choices
I added 4 different shape classes (Cube, Sphere, Cylinder, Cone) that each handle their own tessellation and vertex data generation. UpdateParams() in each of these classes is what is responsible for rebuilding geometry based on tessellation parameters. I also added a camera class which has my view and projection matrix calculations. I included the recursive depth-first traversal that we used in our lab as well in sceneparser.cpp. I use a single Phong shader with uniform arrays for up to 8 lights and in realtime I have separate buffers for each primitive type and I bind the respective VAO before each draw call so that i can minimize state changes.
